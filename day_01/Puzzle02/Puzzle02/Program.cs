...
var sum = Utils.GetSum(input);

Console.WriteLine($"The sum is {sum}.");

class Utils
{
    static List<int> GetAllIndexesForKeyValPair(string value, KeyValuePair<string, int> keyValuePair, List<int> existingIndexes)
    {
        var numIndex = value.LastIndexOf(keyValuePair.Key);
        var valIndex = value.LastIndexOf(keyValuePair.Value.ToString());

        if (numIndex > -1)
        {
            existingIndexes.Add(numIndex);
        }

        if (valIndex > -1)
        {
            existingIndexes.Add(valIndex);
        }

        if (numIndex == -1 && valIndex == -1)
        {
            return existingIndexes;
        }

        var highestIndex = Math.Max(valIndex, numIndex);
        
        return GetAllIndexesForKeyValPair(value.Remove(highestIndex, value.Length - highestIndex), keyValuePair, existingIndexes);
    }
    ...
    public static Tuple<int, int> GetFirstAndLast(string value)
    {
        ...
        {
            indexMap.Add(keyValuePair.Value, GetAllIndexesForKeyValPair(value, keyValuePair, new List<int>()));
        }

        var lowestIndex = value.Length;
        var highestIndex = -1;
        ...
        return new Tuple<int, int>(firstDigit, lastDigit);
    }

    public static int GetSum(List<string> inputs)
    {
        var sum = 0;
